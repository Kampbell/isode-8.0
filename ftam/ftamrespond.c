...
#ifndef	lint
static char *rcsid = "$Header: /xtel/isode/isode/ftam/RCS/ftamrespond.c,v 9.0 1992/06/16 12:14:55 isode Rel $";
#endif

/*
 * $Header: /xtel/isode/isode/ftam/RCS/ftamrespond.c,v 9.0 1992/06/16 12:14:55 isode Rel $
 *
 *
 * $Log: ftamrespond.c,v $
		...
		for (dtn = req -> contents__type__list; dtn; dtn = dtn -> next) {
...
#ifdef COMPAT_OLD_NBS9OID
			/*
			 * Depending on what is supported locally, responder will
			 * accept either old NBS9 document type
			 * OID (1.3.9999.1.5.9) or new one (1.3.14.5.5.9),
			 * but not both!
			 */
			...
			if (!oid_cmp (id -> id_type, str2oid(OLDNBS9_OID))) {
				...
				if (nbs9_marked)
					...
					for (dtn2 = dtn; dtn2; dtn2 = dtn2 -> next) {
						if ((id2 = getisodocumentbytype (dtn2 -> Document__Type__Name))  == NULL)
							continue;
						if (!oid_cmp (id2 -> id_type, str2oid(NEWNBS9_OID)))
							/*
							 * new OID also in the proposed list
							 */
							found_nbs9++;
					}
					if (found_nbs9)
						/*
						 * in any case, if new NBS9 type OID proposed,
						 * old type OID is disregarded...
						 */
						continue;
				}
			}
