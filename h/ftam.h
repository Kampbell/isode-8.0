 ...
 * 	- local entity is the initiator: both types are sent out for
 *	  negotiation. Hopefully, only one will be accepted by the responder;
 *	  if it accepts both, we are committed to support both!
 *	- local entity is responder: we accept whatever the initiator sends
 *	  (with respect to locally known types). If it proposes both
 *	  we only accept new type!
 * PS: OLD TYPE SUPPORT SHOULD GO AWAY ASAP!
 * PPS: May not work with shared FTAM library...
 * (sylvain@cli53an.edf.fr 28/01/92)
 */
#define COMPAT_OLD_NBS9OID
#ifdef COMPAT_OLD_NBS9OID
#define NEWNBS9_OID	"1.3.14.5.5.9"
