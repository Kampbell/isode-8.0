...
#ifdef HULA
	,
	PMASK_NOTSUPPORTED,			/* 0x3f */
	PMASK_VERSION				/* 0x40: SPDU_UD */
	| PMASK_SSAP_CALLING
	| PMASK_SSAP_CALLED
	| PMASK_UDATA
#endif
...
#endif
		start_spdu (s, &c, UD_BASE_SIZE);
		If_Set (SMASK_UD_VERSION)
		Put_Item (PI_VERSION, (char *) &s -> s_ud_version);
		If_Set (SMASK_UD_CALLING)
		put2spdu (PI_SSAP_CALLING, s -> s_ud_callinglen,
				  s -> s_ud_calling, &c);
		If_Set (SMASK_UD_CALLED)
		put2spdu (PI_SSAP_CALLED, s -> s_ud_calledlen,
				  s -> s_ud_called, &c);
		/* NB: caller responsible for mapping s -> s_udata to user info */
		break;
...
struct ssapkt *udtsdu2spkt (qb, len)
struct qbuf *qb;
int	len;
{
	register int    li;
	...
	while (pktlen && (s -> s_errno == OK)) {
		...
		case PI_SSAP_CALLING:
			...
			printf ("\n     PISSAPCALLING \n");
#endif
			bcopy (base, s -> s_ud_calling, s -> s_ud_callinglen = li);
			Set (SMASK_UD_CALLING);
			base += li;
			break;
		...
		case PI_SSAP_CALLED:
			...
			printf ("\n     PISSAPCALLED \n");
#endif
			bcopy (base, s -> s_ud_called, s -> s_ud_calledlen = li);
			Set (SMASK_UD_CALLED);
			base += li;
			break;
...
register struct ssapkt *s;
{
	...
	case SPDU_UD:
		/*
		     *  Do not free any data since the user will free it.
		 */
