...
#ifndef	lint
static char *rcsid = "$Header: /xtel/isode/isode/others/quipu/image/RCS/xwho.c,v 9.0 1992/06/16 12:43:24 isode Rel $";
#endif

/*
 * $Header: /xtel/isode/isode/others/quipu/image/RCS/xwho.c,v 9.0 1992/06/16 12:43:24 isode Rel $
 *
 *
 * $Log: xwho.c,v $
...
char  **vec;
{
	...
	for (vec++; ap = *vec; vec++)
		if (*ap == '-')
			...
			geometry = ap;
		else if ((cp = rindex (ap, ':')) && sscanf (++cp, "%d", &n) == 1)
			display = ap;
		else {
			if (nhosts++ >= NHOSTS)
...

	{
			...
			if (j > 1) {
				...
				for (fp = hp -> h_faces, fq = faces;
						*fq = fp;
						fp = fp -> f_next, fq++)
					continue;
				qsort ((char *) faces, j, sizeof *faces, (IFP)facecmp);
		...
		if (i > 1) {
			...
			for (hp = hosts, hq = hostz; *hq = hp; hp = hp -> h_next, hq++)
				continue;
			qsort ((char *) hostz, i, sizeof *hostz, (IFP)hostcmp);
