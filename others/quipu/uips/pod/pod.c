...
void make_photo_widget() {
	...
	count++;
	XtSetArg(args[count], XtNforeground, WhitePixelOfScreen(screen));
	count++;
	XtSetArg(args[count], XtNbackground, BlackPixelOfScreen(screen));
	count++;
	XtSetArg(args[count], XtNwidth, (Dimension) maxx);
	count++;
	XtSetArg(args[count], XtNheight, (Dimension) py);
	count++;
...
void print_photo() {
	...
	count = 0;
	if (photo_pixmap) {
		XtSetArg(args[count], XtNbackgroundPixmap, photo_pixmap);
		count++;
	}
...
char *top_mess, *lower_mess;
{
	...
	count++;
	XtSetArg(args[count], XtNwidth, Keep_width);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNallowVert, FALSE);
	count++;
	XtSetArg(args[count], XtNallowHoriz, TRUE);
	count++;
...
Widget PosWindow;
{
	...
	count++;
	XtSetArg(args[count], XtNborderColor, WhitePixelOfScreen(screen));
	count++;
	XtSetArg(args[count], XtNhighlightThickness, 1);
	count++;
	XtSetArg(args[count], XtNjustify, XtJustifyLeft);
	count++;
	XtSetArg(args[count], XtNwidth, width);
	count++;
	...
	while (str > (char *) friendly_base_path) {
		...
		*end = '\0';
		...
		XtSetArg(args[count], XtNwidth, width);
		count++;
		XtSetArg(args[count], XtNfromVert, last_rdn);
		count++;
...
Widget parent;
{
	...
	while (av_typeindx[n] != -1) {
		count = 0;
		XtSetArg(args[count], XtNlabel, (String) filttype[av_typeindx[n]]);
		count++;
...
Widget parent;
{
	...
	count++;
	XtSetArg(args[count], XtNwidth, Quit_width);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, Help_width);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, Search_width);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, List_width);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, History_width);
	count++;
...
char *entry_name;
{
	...
	if (curr_read_popup) {
		...
		if (*friendly_name == '\0') {
			...
			XtSetArg(args[count], XtNlabel, friendly_name);
			count++;
		}
...
static Widget createReadPopup() {
	...
	count++;
	XtSetArg(args[count], XtNwidth, ShowAll_width);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, Modify_width);
	count++;
...
char *entry_ptr;
{
	...
	count++;
	XtSetArg(args[count], XtNwidth, max_attr_width);
	count++;
	XtSetArg(args[count], XtNheight, height);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, max_val_width);
	count++;
	XtSetArg(args[count], XtNheight, height);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNwidth, sep_width);
	count++;
	XtSetArg(args[count], XtNheight, height);
	count++;
...
Brunel University";
	...
	count++;
	XtSetArg(args[count], XtNfromVert, title);
	count++;
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	...
	XtSetArg(args[count], XtNlabel, "");
	count++;
	XtSetArg(args[count], XtNfromVert, version_label);
	count++;
	XtSetArg(args[count], XtNwidth, work_width);
	count++;
	XtSetArg(args[count], XtNheight, work_height);
	count++;
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	...
	count++;
	XtSetArg(args[count], XtNfromVert, piccy);
	count++;
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
...
XtPointer closure, calldata;
{
	...
	if (file) {
		...
		XtSetArg(args[count], XtNheight, height);
		count++;
		XtSetArg(args[count], XtNstring, help_buf);
		count++;
		XtSetValues(text, args, count);
	}
}
...
Cardinal width, height;
{
	...
	XtSetArg(args[count], XtNwidth, width);
	count++;
	XtSetArg(args[count], XtNbackgroundPixmap, bitmap);
	count++;
	XtSetArg(args[count], XtNresizable, FALSE);
	count++;
...
XtPointer closure, calldata;
{
	...
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	XtSetArg(args[count], XtNwidth, menuicon_width);
	count++;
	XtSetArg(args[count], XtNheight, menuicon_height);
	count++;
	XtSetArg(args[count], XtNvertDistance, 10);
	count++;
	...
	XtSetArg(args[count], XtNresizable, TRUE);
	count++;
	XtSetArg(args[count], XtNresize, XawtextResizeBoth);
	count++;
	XtSetArg(args[count], XtNstring, "");
	count++;
	XtSetArg(args[count], XtNcallback, modCallbacks);
	count++;
	XtSetArg(args[count], XtNheight, height);
	count++;
	XtSetArg(args[count], XtNfromVert, fromVertWidg);
	...
	count++;
	XtSetArg(args[count], XtNvertDistance, 2);
	count++;
...
XtPointer closure, calldata;
{
	...
	if (this_val->value && *this_val->value != '\0') {
		if (!thisValWidg) {
			...
			XtSetArg(args[count], XtNfromVert, prevValWidg);
			count++;
			XtSetArg(args[count], XtNfromHoriz, thisValMenuWidg);
			count++;
			XtSetArg(args[count], XtNstring, this_val->value);
			count++;
			XtSetArg(args[count], XtNeditType, XawtextEdit);
			count++;
			XtSetArg(args[count], XtNresize, XawtextResizeBoth);
			...
			XtSetArg(args[count],
					 XtNwidth,
					 (Dimension) GetTextWidth(thisValWidg,this_val->value));
			count++;
			XtSetArg(args[count],
					 XtNheight,
			...
			XtSetArg(args[count],
					 XtNwidth,
					 GetTextWidth(thisValWidg, this_val->value));
			count++;
			XtSetArg(args[count],
					 XtNheight,
					 GetTextHeight(thisValWidg, this_val->value));
			count++;
			XtSetArg(args[count], XtNstring, this_val->value);
			count++;
			XtSetValues(thisValWidg, args, count);
		...
		for (curr_val = curr_attrs->val_seq;
			...
			if (curr_val->value)
				if (!curr_attrs->hidden_flag) {
					XtSetArg(args[count], XtNstring, curr_val->value);
					count++;
				} else {
			...
			if (text_height && text_width) {
				XtSetArg(args[count], XtNwidth, text_width);
				count++;
				XtSetArg(args[count], XtNheight, text_height);
				count++;
