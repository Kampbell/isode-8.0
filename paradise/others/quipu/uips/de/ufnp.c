/* ufnp.c - user-friendly name resolution */
...
int leafFlag;
{
	...
	if (exact) {
		...
		*dlist = exact;
		if (leafFlag && (countMatches(exact) > maxPersons)) {
			*dlist = (*interact)(exact,el->dns_dn,s);
			if (*dlist) {
				dn_seq_free (good);
				dn_seq_free (bad);
				return TRUE;
	...
	if (good) {
		...
		*dlist = good;
		if (leafFlag && (countMatches(good) > maxPersons)) {
			*dlist = (*interact)(good,el->dns_dn,s);
			if (*dlist) {
				dn_seq_free (bad);
				return TRUE;
			}
...
set_bad_dsa:
	...
	*res = r;
	...
	return dnSelect (s,res,interact,el, leafFlag);
}
