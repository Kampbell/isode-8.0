...
struct namelist * lp;
{
	...
	for (at = lp->ats; at != NULLATTR; at = at->attr_link) {
		...
		if ((strcmp(attr2name(at->attr_type, OIDPART), "objectClass"))) {
			/* Only print, if it is not the object class, *** HACK *** */
			...
			for (av = at->attr_value; av != NULLAV; av = av->avseq_next) {
				...
				if (tmp_org == TRUE) {
					if (is_dn(attr2name(at->attr_type, OIDPART)) == TRUE) {
						cp = rindex(cp, '\n');
						if (cp != NULLCP) {
							cp = TidyString(cp);
						}
...
printDetailsNumbers(objectType, lp, count)
int objectType;
struct namelist * lp;
int * count;
{
	...
	*count = 0;
	...
	for (at = lp->ats; at != NULLATTR; at = at->attr_link) {
		...
		if ((strcmp(attr2name(at->attr_type, OIDPART), "objectClass"))) {
			/* Only print, if it is not the object class, *** HACK *** */
			(*count)++;
			pageprint("%s%d  %-*s", padding, *count, PADCHARS-3,
					  mapAttName(attr2name(at->attr_type, OIDPART)));
			for (av = at->attr_value; av != NULLAV; av = av->avseq_next) {
				if (av != at->attr_value) {
					pageprint("%s%*s", padding, PADCHARS, "");
...
int format;
{
	if (*str == T61_MARK) {
		if (format != READOUT) {
			ps_print (ps,"{T.61}");
			octprint (ps,mapPhone(++str),format);
		} else if (ch_set == 1)
			iso8859print(ps,mapPhone(++str));
		else
			octprint(ps,mapPhone(++str),format);
	} else {
		if ((*str == ' ') && (*(str+1) == 0))
...
static	de_fax_print (ps, f, format)
register PS ps;
register struct fax *f;
int	format;
{
	...
	if (format == READOUT) {
		ps_printf (ps, "%s", (char *) mapPhone(f -> number));
