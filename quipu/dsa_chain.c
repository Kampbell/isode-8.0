...
DN dsadn;
{
	...
	if ( ! refer_ok) {
		...
		if ((tk->tk_conn->cn_ctx != DS_CTX_X500_DAP)
			...
			DLOG (log_dsap,LLOG_DEBUG,( "Not chaining because dspchaining off"));
			return(FALSE);
		}
...
char ctx;
{
	...
	case DI_COMPLETE:
		...
		break;
	case DI_DEFERRED:
		LLOG(log_dsap, LLOG_TRACE, ("di2cref - Trying to turn deferred di_block into continuation reference!"));
		return(NOTOK);
	default:
...
struct oper_act * on;
{
	...
	for (avs = as->attr_value; avs != NULLAV; avs=avs->avseq_next) {
		...
		case DS_X500_ERROR:
			...
			DLOG(log_dsap, LLOG_TRACE, ("relay - get_dsa_info (slave) returned X500 ERROR"));
			ds_error_free(&err);
			break;
...
char * str;
{
	...
	if (relays_for == NULLDNSEQ)
		...
		for (loop = relays_for; loop->dns_next != NULLDNSEQ; loop=loop->dns_next)
			;
		loop->dns_next = dsa;
	}
}
...
struct di_block * di ;
{
	...
	while(tmp_avs != NULLAV) {
		if ((((struct quipu_call *)tmp_avs->avseq_av.av_struct)->assoc_id == task->tk_conn->cn_ad) &&
				(((struct quipu_call *)tmp_avs->avseq_av.av_struct)->finish_time == (char *) 0)) {
			/* Correct association and it is still open */
			/* Now find the appropriate task */
			...
			while(tmp_op_list != (struct op_list *) 0) {
				if (tmp_op_list->operation_list->invoke_id == task->tk_dx.dx_id) {
					struct oper_act * tmp_operlist = task->tk_operlist ;
					while (tmp_operlist->on_next_conn != (struct oper_act *) 0) {
						...
						if (tmp_op_list->operation_list->chained_ops == (struct chain_list *) 0) {
							...
							tmp_chain_list = tmp_op_list->operation_list->chained_ops ;
							while (tmp_chain_list->next != (struct chain_list *) 0) {
								tmp_chain_list = tmp_chain_list->next ;
							}
							tmp_chain_list->next = new_chain_list ;
						}
						new_chain_list->sub_chained_ops = new_sub_ch_list ;
						new_chain_list->next = (struct chain_list *) 0 ;
						new_sub_ch_list->assoc_id = tmp_operlist->on_conn->cn_ad ;
						new_sub_ch_list->invok_id = tmp_operlist->on_id ;
						tmp_operlist = tmp_operlist->on_next_conn ;
					}
					return ; /* If we had a successful operation match, return. */
				}
				tmp_op_list = tmp_op_list->next ;
			}
			return ; /* If we had a successful call match, return. */
		}
		tmp_avs = tmp_avs->avseq_next ;
	}
#ifdef SPT_DEBUG
	fprintf(stderr, "No match in adding a chained op into open_call_avs.\n") ;
#endif
}
#endif /* QUIPU_CONSOLE */
