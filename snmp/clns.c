...
#undef	clnpRoutingDiscards /* 22	/* NOT IMPLEMENTED */
...
int	offset;
{
	...
	case type_SNMP_PDUs_set__request:
		...
		if (getkmem (nl + N_ISO_SYSTYPE, (caddr_t) &iso_systype,
					 sizeof iso_systype) == NOTOK
				|| chekmem (nl + N_ISO_SYSTYPE) == NOTOK)
			return int_SNMP_error__status_genErr;
	...
	case type_SNMP_PDUs_commit:
		if (setkmem (nl + N_ISO_SYSTYPE, (caddr_t) &iso_systype,
					 sizeof iso_systype) == NOTOK)
			return int_SNMP_error__status_genErr;
	/* and fall */
...
#define	clnpRouteMetric5 10
#define	clnpRouteInfo	11
...
#define	clnpMediaToNetAddress 7
...
init_clns () {
...
#endif
#ifdef	clnpRoutingDiscards
	if (ot = text2obj ("clnpRoutingDiscards"))
		ot -> ot_getfnx = o_clnp,
	...
	if (ot = text2obj ("clnpMediaToNetAddress"))
		ot -> ot_getfnx = o_address,
			  ot -> ot_info = (caddr_t) clnpMediaToNetAddress;
