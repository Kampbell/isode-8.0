...
char  **value;
{
	...
	if (hp = gethostbystring (getlocalhost ())) {
		char   *addr;
...
char   *name;
{
	...
	if (ot = text2obj (name)) {
		if (ot -> ot_syntax) {
			...
			fatal ("SNMP variable \"%s\" has unknown syntax", ot -> ot_text);
	}
	*cp = c;
}
...
struct search *lookat;
{
	...
	lookat -> retval = NULL;
	if (!ot -> ot_getfnx && instance)
		fatal ("can't use SNMP scalar variable \"%s\" as control for for-in",
			   ot -> ot_text);
...
PE	pe;
{
	...
	*x = make_string (ipaddr, strlen (ipaddr));
