...
#endif
#endif
static int  ros_invokerequest ();
static int  do_ros ();
static int  ros_main ();
static int  rts_waitfor ();
static int  rts_main ();
static ps_abort (), ps_event ();
static int  ps_waitfor ();
static int  ps_datarequest ();
static int  ps_main ();
static ss_event ();
static int  ss_waitfor ();
static int  ss_datarequest ();
static int  ss_main ();
static int  ts_datarequest ();
static int  ts_main ();
static int  qcmp ();
static int  rts_event ();
static int  rts_transferequest ();
	...
	expedited;
{
	...
	if (mode == echo) {
		...
		if (cc != tx -> tx_cc) {
			...
			status++;
		} else if (qcmp (data, &tx -> tx_qbuf, cc))
			status++;
		TXFREE (tx)
	}
}
		...
		for (;;) {
			...
			case OK:
				...
				if (cc != sx -> sx_cc) {
					...
					status++;
				} else if (qcmp (data, &sx -> sx_qbuf, cc))
					status++;
				SXFREE (sx)
				break;
...
char   *addr;
{
	...
	if (isacs) {
		...
		ode = oid_cpy (ode);
	} else if ((pz = is2paddr (addr, NULLCP, is)) == NULL)
		adios (NULLCP, "address translation failed");
...
struct PSAPindication *pi;
{
	...
	struct AcSAPindication *aci = &acis;
	struct AcSAPabort *aca = &aci -> aci_abort;
	struct AcSAPfinish *acf = &aci -> aci_finish;
...
char *addr;
{
	...
	if (isacs) {
		...
		pe_free (pe);
	} else if (RtEndRequest (sd, rti) == NOTOK)
		switch (rta -> rta_reason) {
		case RTS_OPERATION:
		case RTS_WAITING:
			rts_waitfor (sd);
...
char *addr;
{
	...
	struct RoSAPaddr  *roa = &roas;
	struct RoSAPconnect rocs;
	struct RoSAPconnect   *roc = &rocs;
	struct RoSAPindication  rois;
...
int	sd;
{
	...
	if (isrts) {
		...
		ACRFREE (acr);
	} else if (RoEndRequest (sd, ROS_NOPRIO, roi) == NOTOK)
