...
#ifndef	lint
static char *rcsid = "$Header: /xtel/isode/isode/support/RCS/isod.c,v 9.0 1992/06/16 12:40:09 isode Rel $";
#endif

/*
 * $Header: /xtel/isode/isode/support/RCS/isod.c,v 9.0 1992/06/16 12:40:09 isode Rel $
 *
 *
 * $Log: isod.c,v $
...
static int	ps_dataindication (), ps_tokenindication (), ps_syncindication (),
...
static int  ros_finish ();
static int  ros_end ();
static int  ros_preject ();
static int  ros_ureject ();
static int  ros_error ();
static int  ros_result ();
static int  ros_invoke ();
static int  do_ros ();
static int  ros_main ();
static int  rts_finish ();
static int  rts_close ();
static int  rts_abort ();
static int  rts_transfer ();
static int  rts_turn ();
static int  rts_main ();
static int  ps_main ();
static int  ss_main ();
	  ...
	  **envp;
{
	...
	if (cp == NULL || strcmp (cp, "tsap") == 0)
		...
		ss_main (argc, argv);
	else if (strcmp (cp, "psap") == 0)
		ps_main (argc, argv);
	else if (strcmp (cp, "acsap") == 0) {
		isacs++;
		ps_main (argc, argv);
	} else if (strcmp (cp, "rtsap") == 0) {
		isrts++;
		rts_main (argc, argv);
	} else if (strcmp (cp, "rosap") == 0)
		ros_main (argc, argv);
	else
...
register struct SSAPdata *sx;
{
	...
	case SX_NORMAL:
		...
		} else if (SDataRequest (sd, p = qb2str (&sx -> sx_qbuf), sx -> sx_cc,
								 si) == NOTOK)
...
#endif
	} else if (SUAbortRequest (sd, NULLCP, 0, si) == NOTOK)
		ss_adios (sa, "S-U-ABORT.REQUEST");
	else
...
char  **argv;
{
	...
	} else if (is = getisoserventbyselector ("psap", ps -> ps_called.pa_selector,
...
register struct PSAPdata *px;
{
	...
	case SX_NORMAL:
		...
		} else if (PDataRequest (sd, px -> px_info, px -> px_ninfo, pi)
				   == NOTOK)
...
register struct PSAPreport *pp;
{
	...
	if (srequirements & SR_DAT_EXISTS) {
		...
		advise (LLOG_DEBUG, NULLCP, "cleared");
#endif
	} else if (PUAbortRequest (sd, NULLPEP, 0, pi) == NOTOK)
		ps_adios (pa, "P-U-ABORT.REQUEST");
	else
		adios (NULLCP, "aborted");
...
char  **argv;
{
	...
	} else if (is = getisoserventbyport ("rtsap", rts -> rts_port))
